...
namespace LineSegmentIntersectorUtils
{
    ...
    {
        TriangleIntersection(unsigned int index, const osg::Vec3& normal, float r1, const osg::Vec3* v1, float r2, const osg::Vec3* v2, float r3, const osg::Vec3* v3, float r, const osg::Vec3 intersection = osg::Vec3(0,0,0)):
            _index(index),
            _normal(normal),
            _r1(r1),
            _v1(v1),
    ...
    struct TriangleIntersector
    {
		...
		inline void operator () (const osg::Vec3& v1, bool treatVertexDataAsTemporary)
		{
			...
			osg::Vec3 c = _d*(v*_d);
 			osg::Vec3 l = v - c;
 			value_type d = l.length();
			if( _ratio>0 && _ratio<FLT_MAX && d>_ratio ) return;
			...
			value_type r = (v*_d)/_length;
			if (r<0.0f) return;
			if (r>1.0f) return;
			if( _intersections->size()>0 && d>_intersections->begin()->first )
				return;			
			...
			{
				_intersections->insert(std::pair<const float,TriangleIntersection>(d,TriangleIntersection(_index-1,normal,1.0f,0,0.0f,0,0.0f,0,r,in)));
			}
			else
			{
				_intersections->insert(std::pair<const float,TriangleIntersection>(d,TriangleIntersection(_index-1,normal,1.0f,&v1,0.0f,0,0.0f,0,r,in)));
			}
			_hit = true;
		}

		inline bool Intersect1(double X1,double Y1,double X2,double Y2,double X3,double Y3,double X4,double Y4,double& X,double& Y)
		{
			double a1=Y2-Y1;
			double b1=X1-X2;
			double X0=0.0;
		...
		inline float lineIntersect(const osg::Vec3& v1, const osg::Vec3& v2, const osg::Vec3& v3, const osg::Vec3& v4, Vec3& in1, Vec3& in2) 
		{
			float d = -1.0;
			double x, y;
			if( !Intersect1(v1.x(), v1.y(),
				v2.x(), v2.y(),
				v3.x(), v3.y(),
				v4.x(), v4.y(),
				x, y) )
			...
			value_type dis1 = sqrt((x - v1.x())*(x - v1.x()) + (y - v1.y())*(y - v1.y())) / sqrt((v2.x() - v1.x())*(v2.x() - v1.x()) + (v2.y() - v1.y())*(v2.y() - v1.y()));
			value_type dis2 = sqrt((x - v3.x())*(x - v3.x()) + (y - v3.y())*(y - v3.y())) / sqrt((v4.x() - v3.x())*(v4.x() - v3.x()) + (v4.y() - v3.y())*(v4.y() - v3.y()));
			in1 = v1 + (v2 - v1)*dis1;
			in2 = v3 + (v4 - v3)*dis2;
			return (in2 - in1).length();
		}
		...
		inline void operator () (const osg::Vec3& v1,const osg::Vec3& v2, bool treatVertexDataAsTemporary)
		{
			...
			Vec3 in1, in2;
			Vec3 e = _s + _d*_length;
			value_type d = lineIntersect(v1, v2, _s, e, in1, in2);
			if( _ratio>0 && _ratio<FLT_MAX && d>_ratio ) return;
			...
			value_type r = (in2-_s).length()/_length;///_length;//ds12;//(_length - d)/_length;
			if (r<0.0f) return;
			if (r>1.0f) return;
			if( _intersections->size()>0 && d>_intersections->begin()->first )
				return;
			...
			if (treatVertexDataAsTemporary)
			{
				_intersections->insert(std::pair<const float,TriangleIntersection>(d,TriangleIntersection(_index-1,normal,1.0f,0,0.0f,0,0.0f,0,r,in1)));
			}
			else
			{
				_intersections->insert(std::pair<const float,TriangleIntersection>(d,TriangleIntersection(_index-1,normal,1.0f,&v1,1.0f,&v2,0.0f,0,r,in1)));
			}
			_hit = true;
		}
        ...
        inline void operator () (const osg::Vec3& v1,const osg::Vec3& v2,const osg::Vec3& v3, bool treatVertexDataAsTemporary)
        {
			...
			if( _intersections->size()>0 && r>_intersections->begin()->first )
				return;

			_intersections->clear();
            if (treatVertexDataAsTemporary)
            {
                _intersections->insert(std::pair<const float,TriangleIntersection>(r,TriangleIntersection(_index-1,normal,r1,0,r2,0,r3,0,r)));
            }
            else
            {
                _intersections->insert(std::pair<const float,TriangleIntersection>(r,TriangleIntersection(_index-1,normal,r1,&v1,r2,&v2,r3,&v3,r)));
            }
            _hit = true;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
//  LineSegmentIntersectorEx
//

LineSegmentIntersectorEx::LineSegmentIntersectorEx(const osg::Vec3d& start, const osg::Vec3d& end):
    _parent(0),
    _start(start),
    _end(end),
	_buffer(2.0f)
...
LineSegmentIntersectorEx::LineSegmentIntersectorEx(CoordinateFrame cf, const osg::Vec3d& start, const osg::Vec3d& end,
                                               LineSegmentIntersectorEx* parent, osgUtil::Intersector::IntersectionLimit intersectionLimit):
    Intersector(cf),
    _parent(parent),
    _start(start),
	_end(end),
	_buffer(2.0f)
{
	setIntersectionLimit(intersectionLimit);
	if( _parent!=nullptr )
		_buffer = _parent->getBuffer();
}

LineSegmentIntersectorEx::LineSegmentIntersectorEx(CoordinateFrame cf, double x, double y):
    Intersector(cf),
	_parent(0),
	_buffer(2.0f)
{
...
Intersector* LineSegmentIntersectorEx::clone(osgUtil::IntersectionVisitor& iv)
{
    ...
    osg::ref_ptr<LineSegmentIntersectorEx> lsi = new LineSegmentIntersectorEx(_start * matrix, _end * matrix);
    lsi->_parent = this;
    lsi->_intersectionLimit = this->_intersectionLimit;
    //lsi->setPrecisionHint(getPrecisionHint());
    return lsi.release();
...
osg::Matrix LineSegmentIntersectorEx::getTransformation(IntersectionVisitor& iv, CoordinateFrame cf)
{
    osg::Matrix matrix;
    switch (cf)
    {
...
bool LineSegmentIntersectorEx::enter(const osg::Node& node)
{
    if (reachedLimit()) return false;
    return !node.isCullingActive() || intersects( node.getBound() );
}

void LineSegmentIntersectorEx::leave()
{
    // do nothing
}

void LineSegmentIntersectorEx::intersect(osgUtil::IntersectionVisitor& iv, osg::Drawable* drawable)
{
                                       ...
                                       const osg::Vec3d& s, const osg::Vec3d& e)
{
    ...
    if (kdTree)
    {
        ...
        {
			osg::Matrix VPW = (*(iv.getViewMatrix())) * (*(iv.getProjectionMatrix())) * (*(iv.getWindowMatrix()));
			osg::Vec3 startWindow = (_start*(*iv.getModelMatrix())) * VPW;
                ...
                ++itr)
            {
				...
				osg::Vec3d localIntersectionPoint = _start*(1.0-remap_ratio) + _end*remap_ratio;
				int type = 0;
				if (geometry)
				{
					switch( geometry->getPrimitiveSet(0)->getMode() )
					{									
					case(GL_LINES):
					case(GL_LINE_STRIP):
					case(GL_LINE_LOOP):
						type = 1;
						localIntersectionPoint = lsi.intersectionPoint;
						break;
				...
				osg::Vec3 window = (localIntersectionPoint*(*iv.getModelMatrix())) * VPW;
				float dis = floor(sqrt((startWindow.x()-window.x())*(startWindow.x()-window.x()) + (startWindow.y()-window.y())*(startWindow.y()-window.y())));
				if( _buffer>0 && dis>=_buffer )
				{
					localIntersectionPoint = _start*(1.0-remap_ratio) + _end*remap_ratio;
					continue;
				}

				remap_ratio = window.z();				
				...
				{				
					...
					if( _buffer>0 )
					{
						if( type==1 )
						{
							if( dis<getIntersections().begin()->distance )
								bNearest = true;
							else if( fabs(dis - getIntersections().begin()->distance)<=std::numeric_limits<double>::epsilon() && remap_ratio<getIntersections().begin()->ratio )
								bNearest = true;
							else if( dis<=_buffer && fabs(dis - getIntersections().begin()->distance)<=_buffer && remap_ratio<getIntersections().begin()->ratio )
								bNearest = true;
						}
						else if( getIntersections().begin()->type==1 )
						{
							if( dis<=_buffer && fabs(dis - getIntersections().begin()->distance)<=_buffer && remap_ratio<getIntersections().begin()->ratio )
								bNearest = true;
						}
						else
						{
...
#else
					bNearest = remap_ratio<getIntersections().begin()->ratio;
#endif
                ...
                hit.localIntersectionPoint = localIntersectionPoint;
    ...
    /*if (getPrecisionHint()==USE_DOUBLE_CALCULATIONS)
    {
        ...
        osg::TriangleFunctorEx< TriangleIntersector > ti;
		osg::Matrix WPVM = osg::Matrix::inverse(*iv.getWindowMatrix()) 
			* osg::Matrix::inverse(*iv.getProjectionMatrix()) 
			* osg::Matrix::inverse(*iv.getViewMatrix()) 
			* osg::Matrix::inverse(*iv.getModelMatrix());
		osg::Vec3d::value_type ratio = _buffer>0 ? (osg::Vec3(0.0, 0.0, 0.0) * WPVM - osg::Vec3(_buffer, _buffer, _buffer) * WPVM).length() : FLT_MAX;
	...
	*/
    {
		...
		osg::Matrix WPVM = osg::Matrix::inverse(*iv.getWindowMatrix()) 
			...
			* osg::Matrix::inverse(*iv.getViewMatrix()) 
			* osg::Matrix::inverse(*iv.getModelMatrix());
		osg::Vec3f::value_type ratio = _buffer>0 ? (osg::Vec3(0.0, 0.0, 0.0) * WPVM - osg::Vec3(_buffer, _buffer, _buffer) * WPVM).length() : FLT_MAX;
        ti.set(&intersections);
        ti.set(s,e,ratio);
        ti._limitOneIntersection = (_intersectionLimit == LIMIT_ONE_PER_DRAWABLE || _intersectionLimit == LIMIT_ONE);
        drawable->accept(ti);
    ...
    if (!intersections.empty())
    {
        ...
        {
            // get ratio in s,e range
            double ratio = thitr->second._ratio;//thitr->first;
			...
			if (geometry)
			{
				...
				case(GL_LINE_LOOP):
					type = 1;
					localIntersectionPoint = thitr->second._intersection;
					break;
			...
			osg::Vec3 window = (localIntersectionPoint*(*iv.getModelMatrix())) * VPW;
			float dis = sqrt((startWindow.x()-window.x())*(startWindow.x()-window.x()) + (startWindow.y()-window.y())*(startWindow.y()-window.y()));
			if( _buffer>0 && dis>_buffer )
				continue;
            ...
            {				
				...
				if( _buffer>0 )
				{
					if( type==1 )
					{
						if( dis<getIntersections().begin()->distance )
							bNearest = true;
						else if( fabs(dis - getIntersections().begin()->distance)<=_buffer && remap_ratio<getIntersections().begin()->ratio )
							bNearest = true;
					}
					else if( getIntersections().begin()->type==1 )
					{
						if( fabs(dis - getIntersections().begin()->distance)<=_buffer && remap_ratio<getIntersections().begin()->ratio )
							bNearest = true;
					}
					else
					{
...
void LineSegmentIntersectorEx::reset()
{
    Intersector::reset();
...
bool LineSegmentIntersectorEx::intersects(const osg::BoundingSphere& bs)
{
...
bool LineSegmentIntersectorEx::intersectAndClip(osg::Vec3d& s, osg::Vec3d& e,const osg::BoundingBox& bbInput)
{
...
osg::Texture* LineSegmentIntersectorEx::Intersection::getTextureLookUp(osg::Vec3& tc) const
{
